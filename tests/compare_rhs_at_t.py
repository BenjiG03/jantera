...
from canterax.thermo import compute_mixture_props, get_h_RT
...
def compare_at_t():
    ...
    print(f"=" * 60)
    print(f"T_Canterax: {T_jt:.4f} K")
    print(f"T_Cantera: {T_ct:.4f} K")
    print(f"dT: {T_jt - T_ct:.4f} K")
    
    # Compare RHS at CANTERAX's state
    print(f"\n--- RHS at Canterax's State ---")
    wdot_jt, h_jt, cp_jt, rho_jt = compute_wdot(T_jt, P0, Y_jt, mech)
    
    # Compute dT/dt for Canterax
    ...
    # Now compute Canterax RHS at Cantera's state for apples-to-apples
    ...
    print(f"  Canterax dT/dt: {dT_dt_jt_at_ct:.4e} K/s")
